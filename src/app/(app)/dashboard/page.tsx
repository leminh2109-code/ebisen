    ...
    <div>
      ...
      {isOwner && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
          <StatCard
            label="Doanh thu năm nay (YTD)"
            amount={summary.ytdRevenue}
          />
          <StatCard
            label="Lãi/Lỗ năm nay (YTD)"
            amount={summary.ytdProfit}
            tone="auto"
          />
        </div>
      )}
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        >
          <p className="font-medium">Xem hóa đơn chi tiết →</p>
          <p className="text-sm text-muted mt-1">
            Danh sách từng hóa đơn, mới nhất trước
          </p>
        </Link>
        <Link
          href="/entry/expense"
        ...
        >
          ...
          <p className="text-sm text-muted mt-1">
            Ghi lại một khoản chi mới
          </p>
        </Link>
      </div>

      <p className="mt-8 text-xs text-muted">
        Mẹo: mọi con số ở đây tính trong Postgres, không tính ở giao diện. Nghĩa
        là bạn có thể hỏi Claude Code bất kỳ câu hỏi nào về dữ liệu và nhận đúng
        con số mà dashboard hiển thị — ví dụ &quot;khách hàng nào doanh thu cao
        nhất {formatMonth(summary.currentMonth)}?&quot;.
      </p>
    </div>
  );
}
